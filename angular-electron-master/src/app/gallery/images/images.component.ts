...
// this.pathArray  = remote.dialog.showOpenDialog({properties: ['openDirectory']});
  }
...
console.log(this.pathArray);
    ...
    if (files && files[0]) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        _formData.append("prlp","tlvininiovnio");
...
// writeFile('txt.txt', files[0].path, function (err) {
...
var savePath = dialog.showSaveDialog({properties: ['openDirectory']});

fs.writeFile(savePath, files[0], function(err) {
    // file saved or err
});
        console.log(JSON.stringify(_formData));
      }
      // var FileSaver = require('asset');
      // FileSaver.saveAs("http://4222", "image.jpg");
  ...
  InsertImages(_formData, lengthFiles) {
   ...
   var string= app.getPath("asset")
    ...
    this.imagesService.InsertImages(_formData, lengthFiles).subscribe((res) => {
     ...
     this.imagesService.urls.push("http://localhost:50637/UploadFile/"+this.imagesService.imageTemp[i].name);
        }
      }
